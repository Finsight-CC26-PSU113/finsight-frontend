...
export const FinancialGoalWidget = () => {
  ...
  const [goalName, setGoalName] = useState(user.financial_goal_name || "");
  const [goalTarget, setGoalTarget] = useState(user.financial_goal_target ? String(user.financial_goal_target) : "");
  const [goalSaved, setGoalSaved] = useState(user.financial_goal_saved ? String(user.financial_goal_saved) : "");
  const [isSaving, setIsSaving] = useState(false);

  useEffect(() => {
    setGoalName(user.financial_goal_name || "");
    setGoalTarget(user.financial_goal_target ? String(user.financial_goal_target) : "");
    setGoalSaved(user.financial_goal_saved ? String(user.financial_goal_saved) : "");
  }, [user.financial_goal_name, user.financial_goal_target, user.financial_goal_saved]);

  const targetAmount = Number(user.financial_goal_target || 0);
      ...
      <Card className="h-full flex flex-col hover:shadow-float transition-all duration-300 border-none shadow-sm ring-1 ring-slate-100">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-500">Kelola target dan progres langsung dari beranda.</p>
          </div>
        </div>
        ...
        <form className="space-y-3 flex-1" onSubmit={handleSave}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input value={goalTarget} onChange={(event) => setGoalTarget(event.target.value)} type="number" min="0" placeholder="0" className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none" />
            </div>
            <div>
          ...
          <div className="pt-1">
            <div className="flex justify-between items-end mb-2 gap-3">
              <span className="text-sm font-medium text-slate-700 truncate">{goalName || "Belum ada tujuan"}</span>
            ...
            <p className="text-xs text-slate-500 text-right">{targetAmount > 0 ? `Tercapai ${progress.toFixed(0)}%` : "Simpan tujuan untuk melihat progres."}</p>
          </div>
