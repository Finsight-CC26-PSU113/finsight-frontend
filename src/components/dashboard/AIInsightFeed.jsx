...
import { Star, AlertTriangle, TrendingUp, Sparkles, Lightbulb } from "lucide-react";
    ...
    <section className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {visibleInsights.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-slate-200/80 bg-white p-5 text-sm text-slate-500">Belum ada insight dari backend.</div>
          ) : (
            visibleInsights.map((item) => {
              const Icon = iconMapper[item.type] || Sparkles;
              return (
                <motion.article key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: item.id * 0.08 }} className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm hover:border-blue-300/40 hover:shadow-xl">
                  <div className="flex items-center justify-between gap-3">
                    <div className="inline-flex h-12 w-12 items-center justify-center rounded-3xl bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-lg">
                      <Icon className="h-5 w-5" />
                    </div>
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{item.type}</span>
                  </div>
      ...
      <div className="rounded-3xl border border-slate-200/80 bg-gradient-to-br from-white via-slate-50 to-cyan-50 p-6 shadow-sm">
        <div className="rounded-3xl bg-slate-900/95 p-6 text-white shadow-xl">
          ...
          <p className="mt-4 text-sm leading-7 text-slate-200/90">FinSight merekomendasikan 3 kategori untuk pengurangan otomatis dan alokasi ulang sisa budget ke tabungan darurat.</p>
