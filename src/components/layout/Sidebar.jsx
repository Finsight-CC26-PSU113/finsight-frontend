    ...
    <aside className={`hidden md:flex flex-col h-screen fixed top-0 left-0 bg-white border-r border-slate-200 z-20 transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
      <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3 px-6'} py-8 relative group`}>
        ...
        <button 
          ...
          className={`absolute ${isCollapsed ? '-right-3' : 'right-4'} top-1/2 -translate-y-1/2 bg-white border border-slate-200 rounded-full p-1 text-slate-400 hover:text-primary-600 hover:border-primary-200 shadow-sm transition-all z-10 ${isCollapsed ? 'opacity-0 group-hover:opacity-100' : ''}`}
        >
      ...
      <nav className={`flex-1 space-y-2 overflow-y-auto mt-2 ${isCollapsed ? 'px-3' : 'px-4'}`}>
        {navItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            className={({ isActive }) =>
              `flex items-center gap-3 py-3 rounded-xl transition-all duration-200 ${isCollapsed ? 'justify-center px-0' : 'px-4'} ${
                isActive
      ...
      <div className={`p-4 border-t border-slate-100 flex flex-col gap-1 ${isCollapsed ? 'px-3' : ''}`}>
        <NavLink
          to="/settings"
          className={({ isActive }) =>
            `flex items-center gap-3 py-3 w-full rounded-xl transition-colors ${isCollapsed ? 'justify-center px-0' : 'px-4'} ${
              isActive 
        ...
        <button 
          onClick={logout}
          className={`flex items-center gap-3 py-3 w-full text-red-500 hover:bg-red-50 rounded-xl transition-colors ${isCollapsed ? 'justify-center px-0' : 'px-4'}`}
