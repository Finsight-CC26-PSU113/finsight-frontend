      ...
      {!isFinished ? (
        ...
        >
          <div>
            ...
            <h2 className="text-lg md:text-2xl font-bold text-slate-900 mt-3 leading-snug md:leading-relaxed">
              {currentQ.question}
            </h2>
          </div>
            ...
            {currentQ.options.map((option, index) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 md:p-4 rounded-xl border-2 cursor-pointer transition-all duration-150 ${
                    isSelected
                      ? 'border-primary-500 bg-primary-50 shadow-sm'
                      : 'border-slate-100 bg-white hover:border-primary-200 hover:bg-slate-50'
                  }`}
                >
                  <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-colors ${
                    isSelected ? 'bg-primary-500 text-white' : 'bg-slate-100 text-slate-500'
                  }`}>
                    {String.fromCharCode(65 + index)}
                  </div>
                  <p className={`text-sm font-medium leading-snug ${isSelected ? 'text-primary-900' : 'text-slate-700'}`}>
                    {option.text}
                  </p>
                </div>
              );
