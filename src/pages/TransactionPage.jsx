...
import { Search, Plus, Tv, Coffee, Zap, Trash2, ShoppingBag, Car, Heart, BookOpen, Plane, HelpCircle, ArrowUpRight, ArrowDownRight, SlidersHorizontal, Calendar, Wallet, Sparkles, Check } from "lucide-react";
...
export const TransactionPage = () => {
  ...
  // Debug: log transactions for troubleshooting empty list issues
  React.useEffect(() => {
    console.debug("[TransactionPage] transactions count:", transactions.length, transactions.slice(0, 3));
  }, [transactions]);
  ...
  // Quick scan: handle file input change, upload to /api/scan and save as transaction
  const handleScanFileChange = async (e) => {
    ...
    try {
      ...
      console.debug("[TransactionPage] uploading scan file", file.name);
      ...
      const grandTotal = Number(scan?.total ?? scan?.totals?.grand_total ?? 0);
      const parsedDate = new Date(scan?.date || new Date().toISOString());
      ...
      await addTransaction({
        title: scan?.store || scan?.merchant || "Struk",
        amount: grandTotal,
        category: "",
        type: "expense",
        date: transactionDate,
        paymentMethod: "cash",
        description: itemNames ? `${scan?.store || ""} - ${itemNames}` : scan?.store || "",
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <input ref={scanInputRef} id="tx-scan-input" type="file" accept="image/*" className="hidden" onChange={handleScanFileChange} />
          <Button type="button" variant="outline" className="flex items-center gap-2 shadow-sm rounded-xl py-2.5 px-3" onClick={() => scanInputRef.current?.click()}>
        ...
        <div className="flex flex-col xl:flex-row gap-4 items-stretch xl:items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="bg-slate-100 p-1 rounded-full flex items-center border border-slate-200/50 shadow-sm shrink-0">
              <button onClick={() => setFilter("all")} className={`px-5 py-2 text-xs md:text-sm font-semibold rounded-full transition-all cursor-pointer ${filter === "all" ? "bg-primary-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-900 hover:bg-slate-50/50"}`}>
                Semua
              </button>
              <button onClick={() => setFilter("income")} className={`px-5 py-2 text-xs md:text-sm font-semibold rounded-full transition-all cursor-pointer ${filter === "income" ? "bg-primary-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-900 hover:bg-slate-50/50"}`}>
                Pemasukan
              </button>
              <button onClick={() => setFilter("expense")} className={`px-5 py-2 text-xs md:text-sm font-semibold rounded-full transition-all cursor-pointer ${filter === "expense" ? "bg-primary-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-900 hover:bg-slate-50/50"}`}>
                Pengeluaran
              </button>
            </div>
            ...
            <div className="relative shrink-0">
              ...
              <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="pl-10 pr-9 py-2.5 bg-white border border-slate-200 rounded-full text-xs md:text-sm font-semibold text-slate-700 outline-none hover:bg-slate-50 transition-all shadow-sm cursor-pointer appearance-none">
              ...
              <div key={dateStr} className="space-y-3">
                ...
                <div className="space-y-3">
                  {txs.map((tx, index) => {
                    ...
                    return (
                      <motion.div key={tx.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="bg-white border border-slate-100 hover:border-slate-200 rounded-2xl p-4 shadow-[0_2px_8px_rgba(0,0,0,0.015)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.035)] transition-all flex items-center justify-between group cursor-pointer">
